...
import { manifest } from "../src/data/templates/to-hong.manifest";
...
try {
  const user = await db.user.create({ data: { email: `to-hong-audit-${Date.now()}@example.invalid`, passwordHash: "" } });
  userId = user.id;
  const seed = fromDemoContent(manifest.demoContent);
  const invitation = await db.invitation.create({ data: {
    userId, templateId: "to-hong", slug: `to-hong-audit-${Date.now()}`, status: "published", publishedAt: new Date(),
  ...
  await page.locator("[data-to-hong-cover]").screenshot({ path: "docs/research/to-hong/320-long-cover.png" });
  await page.locator("[data-to-hong-cover]").getByRole("button", { name: "Mở thiệp" }).click();
  ...
  for (const name of ["portrait", "album", "dress", "gifts", "location", "schedule"]) assert.equal(await article.locator(`[data-to-hong-section="${name}"]`).count(), 0, `${name} hides when empty`);
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  await page.locator('[data-to-hong-section="hero"]').screenshot({ path: "docs/research/to-hong/320-long-hero.png" });
  ...
  await page.getByTestId("to-hong-rsvp-knot").waitFor();
  await page.locator('[data-to-hong-section="guestbook"]').screenshot({ path: "docs/research/to-hong/320-live-forms.png" });
  ...
  await writeFile("docs/research/to-hong/live-audit.json", JSON.stringify({ at: new Date().toISOString(), localOnly: true, longNames320px: true, brideFirst: true, emptySections: true, wishValidation: true, wishPersisted: true, rsvpPersisted: { attending: saved.attending, guests: saved.guests }, editorAccess: true, fixturesDeletedOnExit: true }, null, 2));
