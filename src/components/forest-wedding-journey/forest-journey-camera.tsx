...
export type ForestJourneyLookState = {
  pitchDegrees: number;
  yawDegrees: number;
};

export type ForestJourneyInvalidateBridge = MutableRefObject<
  (() => void) | null
>;

type ForestJourneyCameraProps = {
  ...
  readonly lookRef: MutableRefObject<ForestJourneyLookState>;
...
const ZERO_LOOK: ForestJourneyLookState = {
  pitchDegrees: 0,
  yawDegrees: 0,
};
...
type ForestJourneyCameraDiagnosticCache = {
  cameraX: number;
  cameraY: number;
  cameraZ: number;
...
function writeDiagnostics(
  element: HTMLElement | null,
  camera: Camera,
  look: ForestJourneyLookState,
  cache: ForestJourneyCameraDiagnosticCache,
): void {
  if (!element) return;
...
export function getForestJourneyRailMidpoint(
  sourceScene: ForestJourneyScene,
  targetScene: ForestJourneyScene,
): Vector3 {
  if (Math.abs(sourceScene.ordinal - targetScene.ordinal) !== 1) {
    throw new Error(
      `Forest journey camera travel must connect adjacent scene ordinals: ${sourceScene.id} (${sourceScene.ordinal}) -> ${targetScene.id} (${targetScene.ordinal})`,
    );
  }
  ...
  if (lowerScene.travelMidpointToNext === null) {
    throw new Error(
      `Forest journey camera rail from ${lowerScene.id} (${lowerScene.ordinal}) requires an authored midpoint to the next scene`,
    );
  }
...
function copyCueValues(
  ...
  values: ForestCueValues,
): void {
  cue.doveFlight = values.doveFlight;
  cue.petalGust = values.petalGust;
  cue.rabbitGuide = values.rabbitGuide;
  cue.voileLift = values.voileLift;
  cue.windStrength = values.windStrength;
}

export function ForestJourneyCamera({
  cueRef,
  currentIndex,
  diagnosticsRef,
  invalidateBridgeRef,
  ...
  scenes,
  targetIndex,
}: ForestJourneyCameraProps) {
  const { camera, invalidate } = useThree();
  const timelineRef = useRef<gsap.core.Timeline | null>(null);
  const diagnosticsCacheRef = useRef<ForestJourneyCameraDiagnosticCache>({
    cameraX: Number.NaN,
    cameraY: Number.NaN,
    cameraZ: Number.NaN,
    element: null,
  ...
  useLayoutEffect(() => {
    ...
    if (!scene) {
      throw new Error(
        `Forest journey camera cannot resolve current scene index ${currentIndex}`,
      );
    }
  ...
  useLayoutEffect(() => {
    ...
    if (!sourceScene || !targetScene) {
      throw new Error(
        `Forest journey camera cannot resolve travel indices ${currentIndex} -> ${targetIndex}`,
      );
    }
    const midpoint = getForestJourneyRailMidpoint(sourceScene, targetScene);
    ...
    const renderedLook: ForestJourneyLookState = { ...startLook };
    ...
    const evaluateCue = (progress: number) => {
      const cueValues = evaluateForestCue({
        phase: "travelling",
        progress,
        reducedMotion,
