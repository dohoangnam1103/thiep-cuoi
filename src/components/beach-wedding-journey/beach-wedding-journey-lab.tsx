...
// Copied from src/components/forest-wedding-journey/forest-wedding-journey-lab.tsx, identical
// but for the renamed identifiers and the `beachWeddingJourneyLab` message
// namespace: the entry threshold, the reducer wiring, the WebGL capability probe,
// the look throttle and the fallback handoff are all journey mechanics rather
// than location. Fixes must be applied to both.
...
import { useBeachJourneyLocalInteractions } from "./beach-interactive-scenes";
import {
  BeachJourneyFallback,
} from "./beach-journey-fallback";
import type {
  BeachJourneySceneNames,
  BeachSceneLabels,
} from "./beach-scene-content";
import { useBeachJourneyInput } from "./use-beach-journey-input";
import styles from "./beach-wedding-journey.module.css";

import type { BeachJourneyCanvasProps } from "./beach-journey-canvas";

function BeachJourneyCanvasImportFailure({
  onUnavailable,
...
const BeachJourneyCanvas = dynamic(
  () => import("./beach-journey-canvas")
    .then((module) => module.BeachJourneyCanvas)
    .catch(() => BeachJourneyCanvasImportFailure),
  { ssr: false },
);
...
export type BeachWeddingJourneyFixture = "default" | "long-copy";
...
export function BeachWeddingJourneyLab({
  ...
  const content = useMemo(() => {
    ...
    return {
      ...
      families: {
        ...beachWeddingJourneyDemoContent.families,
      ...
      mapQuery: longCopy
        ...
        : beachWeddingJourneyDemoContent.mapQuery,
      openingMessage: t("demoContent.openingMessage"),
      schedule: beachWeddingJourneyDemoContent.schedule.map((item, index) => ({
        ...item,
        label: scheduleLabels[index] ?? item.label,
      })),
      venueAddress: longCopy
        ? t("longCopy.venueAddress")
        : beachWeddingJourneyDemoContent.venueAddress,
      wishes: beachWeddingJourneyDemoContent.wishes.map((wish, index) => ({
        ...wish,
        message: wishMessages[index] ?? wish.message,
      })),
    };
  }, [fixture, t]);
  const interactions = useBeachJourneyLocalInteractions(
    content.wishes,
  );
  const scenes = useMemo(
    () => buildBeachJourneyScenes(content, beachWeddingJourneyFeatures),
    [content],
  );
  const [state, dispatch] = useReducer(
    beachJourneyReducer,
    scenes.length,
    (sceneCount) => createInitialBeachJourneyState(sceneCount, false),
  );
  ...
  const sceneNames = useMemo<BeachJourneySceneNames>(() => {
    const names: Record<BeachJourneySceneType, string> = {
  ...
  const labels = useMemo<BeachSceneLabels>(() => ({
  ...
  const dispatchJourney = useCallback((action: BeachJourneyAction) => {
    const currentState = stateRef.current;
    const predictedState = beachJourneyReducer(currentState, action);
  ...
  const handleLook = useCallback((pitchDeltaDegrees: number, yawDeltaDegrees: number) => {
    ...
    const nextLook = {
      pitchDegrees: Math.max(
        -beachWeddingJourneyDefinition.look.pitchDegrees,
        Math.min(
          beachWeddingJourneyDefinition.look.pitchDegrees,
      ...
      yawDegrees: Math.max(
        -beachWeddingJourneyDefinition.look.yawDegrees,
        Math.min(
          beachWeddingJourneyDefinition.look.yawDegrees,
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const heading = stageRef.current?.querySelector<HTMLHeadingElement>(
        `[data-beach-scene-id="${scene.id}"][data-beach-interactive="true"] h2`,
      );
      ...
      {showThresholdVoile ? (
        <section className={styles.entry} aria-labelledby="beach-journey-entry-title">
          <div
            ...
            data-testid="beach-journey-voile"
        ...
        <>
          ...
          <nav className={styles.controls}>
            <button
              ...
              data-testid="beach-journey-previous"
            ...
            <button
              className={styles.controlButton}
              data-testid="beach-journey-next"
            ...
            <button
              ...
              className={styles.controlButton}
              data-testid="beach-journey-reduced-motion"
