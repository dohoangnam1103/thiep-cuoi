// Copied from src/components/forest-wedding-journey/forest-journey-controller.ts. Fixes to journey
// mechanics must be applied to both.

import { beachWeddingJourneyDefinition } from "@/data/beach-wedding-journey";

export type BeachJourneyPhase =
  | "threshold"
  | "settled"
  | "travelling"
  | "fallback-settled";

export type BeachNavigationSource = "button" | "gesture" | "keyboard" | "wheel";

export type BeachJourneyState = {
  currentIndex: number;
  look: {
    pitchDegrees: number;
    yawDegrees: number;
  };
  navigationSource: BeachNavigationSource | null;
  phase: BeachJourneyPhase;
...
export type BeachJourneyAction =
  | { source: BeachNavigationSource; type: "enter" | "next" | "previous" }
  | { type: "arrive" }
...
export function createInitialBeachJourneyState(
  sceneCount: number,
  reducedMotion: boolean,
): BeachJourneyState {
  if (sceneCount < 2) {
    throw new Error("Beach journey requires gate and finale");
  }
...
export function beachJourneyReducer(
  state: BeachJourneyState,
  action: BeachJourneyAction,
): BeachJourneyState {
  switch (action.type) {
    case "enter":
