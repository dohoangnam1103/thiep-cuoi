  ...
  return (
    <section className="mt-12 border-t border-border pt-10">
      <div className="max-w-2xl">
        <h2 className={`${panelTitleClass} text-foreground`}>{labels.sectionTitle}</h2>
        <p className={`mt-2 ${bodyClass} text-muted-foreground`}>{labels.sectionDescription}</p>
      </div>
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className={`${panelSubTitleClass} text-foreground`}>{labels.wishesTitle} ({wishes.length})</h3>
          </div>
          {wishes.length === 0 ? (
            <p className={`mt-4 rounded-2xl border border-dashed border-border p-6 ${bodyClass} text-muted-foreground`}>{labels.noWishes}</p>
          ) : (
                ...
                <li key={wish.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className={`${bodyClass} font-semibold text-foreground`}>{wish.name}</p>
                      <p className={`mt-1 ${bodyClass} text-muted-foreground`}>{wish.text}</p>
                    </div>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className={`${panelSubTitleClass} text-foreground`}>{labels.mediaTitle} ({media.length})</h3>
          </div>
          {media.length === 0 ? (
            <p className={`mt-4 rounded-2xl border border-dashed border-border p-6 ${bodyClass} text-muted-foreground`}>{labels.noMedia}</p>
          ) : (
            <ul className="mt-4 grid grid-cols-2 gap-3">
              {media.map((item) => (
                <li key={item.id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                  ...
                  <div className="p-3">
                    <p className={`truncate ${bodyClass} font-semibold text-foreground`}>{item.contributorName}</p>
                    <p className={`mt-0.5 truncate ${bodySmallClass} text-muted-foreground`}>{item.originalName} · {formatSize(item.size)}</p>
                    <div className="mt-3 flex gap-2">
                      <a href={`${item.url}?download=1`} download className={`inline-flex min-h-9 flex-1 items-center justify-center gap-1.5 rounded-full bg-secondary px-3 ${pillClass} text-secondary-foreground transition hover:bg-muted`}>
          ...
          <Dialog.Viewport className="fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4">
            <Dialog.Popup className="w-full max-w-md rounded-t-3xl bg-card p-5 text-card-foreground shadow-2xl outline-none transition data-[ending-style]:translate-y-4 data-[ending-style]:opacity-0 data-[starting-style]:translate-y-4 data-[starting-style]:opacity-0 sm:rounded-3xl sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <Dialog.Title className={panelTitleClass}>
                    {target?.kind === "media" ? labels.deleteMediaTitle : labels.deleteWishTitle}
                  </Dialog.Title>
                  <Dialog.Description className={`mt-2 ${bodyClass} text-muted-foreground`}>
              ...
              {target ? <p className={`mt-4 truncate rounded-xl bg-muted px-3 py-2 ${bodyClass} font-medium`}>{target.label}</p> : null}
              {error ? <p role="alert" className={`mt-4 rounded-xl bg-red-50 px-3 py-2 ${bodyClass} text-red-700`}>{error}</p> : null}
              <div className="mt-6 flex justify-end gap-3">
                <Dialog.Close disabled={pending} className={`min-h-11 rounded-full border border-border px-5 ${bodyClass} font-semibold transition hover:bg-muted disabled:opacity-50`}>{labels.cancel}</Dialog.Close>
                <button type="button" onClick={confirmDelete} disabled={pending} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-red-600 px-5 ${bodyClass} font-semibold text-white transition hover:bg-red-700 disabled:opacity-60`}>
