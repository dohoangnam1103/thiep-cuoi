...
import { Home2Page } from "@/components/home2/home2-page";
...
export const metadata: Metadata = {
  title: "Trang chủ · bản thiết kế 2",
  // Bản thiết kế chạy song song với trang chủ thật. Không cho index để tránh
  // trùng nội dung với "/" và tránh lọt bản chưa chốt vào kết quả tìm kiếm.
...
/** Số mẫu đưa lên trang: 5 cho dải hero, 6 cho lưới chương 01. */
const HERO_COUNT = 5;
const GRID_COUNT = 6;

export default async function HomeV2({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const featured = templates.slice(0, HERO_COUNT + GRID_COUNT);
  ...
  const shots: TemplateShotData[] = await Promise.all(
    featured.map(async (template) => {
      ...
      // Thứ tự ưu tiên tên: admin đổi tên trong DB → tên tiếng Việt trong
      // catalog → tên gốc trong data.
      const name =
