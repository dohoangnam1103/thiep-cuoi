...
test("the dedicated renderer registry contains all 21 unique templates", () => {
  assert.equal(AUDITED_TEMPLATE_SLUGS.length, 21);
  assert.equal(new Set(AUDITED_TEMPLATE_SLUGS).size, 21);
